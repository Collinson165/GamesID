...
const GameCard: React.FC<GameCardProps> = ({ game }) => {
    const year = moment.unix(game.first_release_date).year()
    ...
    return (
        <Link href={`/games/[gameId]`} as={`/games/${game.id}`}>
            ...
            <img src={game.cover ? `https://images.igdb.com/igdb/image/upload/t_cover_big/${game.cover.image_id}.jpg` : `https://images.igdb.com/igdb/image/upload/t_thumb/co5w9p.jpg`} alt={game.name} height="800" width={600} className="w-full" />
            ...
            <div className="px-4 pt-2 pb-1">
                <button className="absolute bg-opacity-40 top-0 right-0 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">
                    {game.aggregated_rating ? Math.round(game.aggregated_rating): 'No Rating'}
                </button>
            </div>
        </div>
        </Link>
